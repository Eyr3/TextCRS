...
from textattacknew.datasets import HuggingFaceDataset
from textattacknew.models.helpers import LSTMForClassification, WordCNNForClassification
from textattacknew.models.wrappers import (
    HuggingFaceModelWrapper,
    ModelWrapper,
    PyTorchModelWrapper,
)
from textattacknew.shared import logger
from textattacknew.shared.utils import ARGS_SPLIT_TOKEN
...
class TrainingArgs:
    ...
    Args:
        ...
        attack_num_workers_per_device (:obj:`int`, defaults to `optional`, :obj:`1`):
            Number of worker processes to run per device for attack. Same as :obj:`num_workers_per_device` argument for :class:`~textattacknew.AttackArgs`.
        ...
        wandb_project (:obj:`str`, `optional`, defaults to :obj:`"textattacknew"`):
    ...
    wandb_project: str = "textattacknew"
    logging_interval_step: int = 1
    if_addnoise: int = 0
    pad: str = "[PAD]"  # lstm: <pad>/[PAD], bert: [PAD]
    pad_token: int = 0
    model_type: str = "bert"
    syn_size: int = 250  # noise_1 / 9
    shuffle_len: int = 300  # noise_2 / 8
    noise_sd: float = 0  # noise_3
    dynamic_mu: bool = True
    use_kl: bool = False
    VSD_loss: float = 0.0
    beta: float = 0  # noise_4
    start_epoch: int = 1
    sigma: float = 0
    N: int = 20000
    num_classes: int = 2
    certify_log: str = None
    test_log: str = None
    skip: int = 100
    certify_batch: int = 200
    continue_idx: int = -1
    ...
    def _add_parser_args(cls, parser):
        ...
        parser.add_argument(
            "--if_addnoise",
            type=int,
            default=default_obj.if_addnoise,
            help="0_no noise; 1_synonyms replace; 2_shuffling noise; 3_add word; 4_delete word.",
        )
        parser.add_argument(
            "--syn_size",
            type=int,
        ...
        parser.add_argument(
            "--shuffle_len",
            type=int,
            default=default_obj.shuffle_len,
            help="the length of the shuffling range",
        )
        parser.add_argument(
            "--noise_sd",
            type=float,
            default=default_obj.noise_sd,
            help="the length of the shuffling range",
        )
        parser.add_argument(
            "--dynamic_mu",
            type=bool,
            default=default_obj.dynamic_mu,
            help="if use dynamic mu",
        )
        parser.add_argument(
            "--use_kl",
            type=bool,
            default=default_obj.use_kl,
            help="if use kl divergence to optimize CompressChannel",
        )
        parser.add_argument(
            "--VSD_loss",
            type=float,
            default=default_obj.VSD_loss,
            help="the weight of VSD",
        )
        parser.add_argument(
            "--beta",
            type=float,
            default=default_obj.beta,
            help="each word has the probability of beta to remain unchanged",
        )
        parser.add_argument(
            "--model_type",
            type=str,
            default=default_obj.model_type,
            help="data type",
        )
        parser.add_argument(
            "--start_epoch",
            type=int,
            default=default_obj.start_epoch,
            help="training start epoch",
        )
        parser.add_argument(
            "--sigma",
            type=float,
            default=default_obj.sigma,
            help="predict/certify noise hyperparameter",
        )
        parser.add_argument(
            "--N",
            type=int,
            default=default_obj.N,
            help="certify N",
        )
        parser.add_argument(
            "--num_classes",
            type=int,
            default=default_obj.num_classes,
            help="number of dataset classes",
        )
        parser.add_argument(
            "--certify_log",
            type=str,
            default=default_obj.certify_log,
            help="certify log path",
        )
        parser.add_argument(
            "--test_log",
            type=str,
            default=default_obj.test_log,
            help="certify log path",
        )
        parser.add_argument(
            "--skip",
            type=int,
            default=default_obj.skip,
            help="how many examples to skip when predict/certify",
        )
        parser.add_argument(
            "--certify_batch",
            type=int,
            default=default_obj.certify_batch,
            help="certify batch size",
        )
        parser.add_argument(
            "--continue_idx",
            type=int,
            default=default_obj.continue_idx,
            help="idx continue to certify",
        )
        return parser
...
class _CommandLineTrainingArgs:
    ...
    def _create_model_from_args(cls, args):
        ...
        if args.model_name_or_path == "lstm":
            ...
            logger.info("Loading textattacknew model: WordCNNForClassification")
            max_seq_len = args.model_max_length if args.model_max_length else 128
            num_labels = args.model_num_labels if args.model_num_labels else 2
    ...
    def _create_attack_from_args(cls, args, model_wrapper):
        import textattacknew  # noqa: F401
        ...
        assert isinstance(
            attack, Attack
        ), "`attack` must be of type `textattacknew.Attack`."
        return attack
