...
TEXTATTACK_DATASET_BY_MODEL = {
    ...
    #
    "t5-en-de": (
        "textattacknew.datasets.helpers.TedMultiTranslationDataset",
        "en",
        "de",
    ),
    "t5-en-fr": (
...
class DatasetArgs:
    ...
    def _create_dataset_from_args(cls, args):
        ...
        ``textattacknew.dataset.Dataset`` object."""

        assert isinstance(
            args, cls
        ), f"Expect args to be of type `{type(cls)}`, but got type `{type(args)}`."

        # Automatically detect dataset for huggingface & textattacknew models.
        ...
        if args.dataset_by_model in HUGGINGFACE_DATASET_BY_MODEL:
            ...
            if dataset[0].startswith("textattacknew"):
                # unsavory way to pass custom dataset classes
                # ex: dataset = ('textattacknew.datasets.helpers.TedMultiTranslationDataset', 'en', 'de')
        ...
        if args.dataset_from_file:
            ...
            if args.dataset_split:
                if len(dataset_args) > 1:
                    ...
                    dataset = textattacknew.datasets.HuggingFaceDataset(
        ...
        ), "Loaded `dataset` must be of type `textattacknew.datasets.Dataset`."
