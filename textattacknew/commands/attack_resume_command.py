...
from textattacknew import Attacker, CommandLineAttackArgs, DatasetArgs, ModelArgs
...
class AttackResumeCommand(TextAttackCommand):
    ...
    def run(self, args):
        ...
        assert isinstance(checkpoint.attack_args, CommandLineAttackArgs), (
            f"Expect `attack_args` to be of type `textattacknew.args.CommandLineAttackArgs`, but got type `{type(checkpoint.attack_args)}`. "
            f"If saved `attack_args` is not of type `textattacknew.args.CommandLineAttackArgs`, cannot resume attack from command line."
        )
    ...
    def _parse_checkpoint_from_args(self, args):
        ...
        checkpoint = textattacknew.shared.AttackCheckpoint.load(checkpoint_path)
