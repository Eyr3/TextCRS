...
logger = textattacknew.shared.logger


class PeekDatasetCommand(TextAttackCommand):
    """The peek dataset module:

    Takes a peek into a dataset in textattacknew.
    """

    def run(self, args):
        UPPERCASE_LETTERS_REGEX = re.compile("[A-Z]")

        dataset_args = textattacknew.DatasetArgs(**vars(args))
        dataset = textattacknew.DatasetArgs._create_dataset_from_args(dataset_args)
        ...
        for inputs, output in dataset:
            at = textattacknew.shared.AttackedText(inputs)
    ...
    def register_subcommand(main_parser: ArgumentParser):
        ...
        parser = textattacknew.DatasetArgs._add_parser_args(parser)
