...
from textattacknew import DatasetArgs, ModelArgs
from textattacknew.commands import TextAttackCommand
from textattacknew.model_args import HUGGINGFACE_MODELS, TEXTATTACK_MODELS

logger = textattacknew.shared.utils.logger


def _cb(s):
    return textattacknew.shared.utils.color_text(str(s), color="blue", method="ansi")
...
class EvalModelCommand(TextAttackCommand):
    ...
    def get_preds(self, model, inputs):
        with torch.no_grad():
            preds = textattacknew.shared.utils.batch_model_predict(model, inputs)
        return preds
    ...
    def test_model_on_dataset(self, args):
        ...
        while i < min(args.num_examples, len(dataset)):
            ...
            for (text_input, ground_truth_output) in dataset_batch:
                attacked_text = textattacknew.shared.AttackedText(text_input)
        ...
        if preds.ndim == 1:
            ...
            # TODO integrate with `textattacknew.metrics` package
    ...
    def run(self, args):
        ...
        textattacknew.shared.utils.set_seed(args.random_seed)
