...
class NEWWordCNNForClassification(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self._config = {
            "architectures": "NEWWordCNNForClassification",
        ...
        # noise3, encoder-decoder layer
        if self._config["channel_rate"]:
            self.channel = ChannelCompress(in_ch=self.emb_layer.n_d, out_ch=self.emb_layer.n_d, dropout=0.3,
                                           channel_rate=self._config["channel_rate"])
    ...
    def forward(self, _input, noise_sd=0, mu=0, if_channel=True):
        ...
        emb = emb + g_noise  # device=torch.cuda.current_device()

        if if_channel and self._config["channel_rate"]:
            emb1 = torch.flatten(emb, end_dim=1)
            emb1 = self.channel(emb1)
            emb = emb1.reshape(emb.size())  # emb1.reshape([128, 100, 300])
