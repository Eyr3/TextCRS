...
from textattacknew.constraints import Constraint, PreTransformationConstraint
from textattacknew.goal_function_results import GoalFunctionResultStatus
from textattacknew.goal_functions import GoalFunction
from textattacknew.models.wrappers import ModelWrapper
from textattacknew.search_methods import SearchMethod
from textattacknew.shared import AttackedText, utils
from textattacknew.transformations import CompositeTransformation, Transformation
...
class Attack:
    ...
    def __init__(
        ...
        assert isinstance(
            goal_function, GoalFunction
        ), f"`goal_function` must be of type `textattacknew.goal_functions.GoalFunction`, but got type `{type(goal_function)}`."
        assert isinstance(
            constraints, list
        ), "`constraints` must be a list of `textattacknew.constraints.Constraint` or `textattacknew.constraints.PreTransformationConstraint`."
        ...
        assert isinstance(
            transformation, Transformation
        ), f"`transformation` must be of type `textattacknew.transformations.Transformation`, but got type `{type(transformation)}`."
        assert isinstance(
            search_method, SearchMethod
        ), f"`search_method` must be of type `textattacknew.search_methods.SearchMethod`, but got type `{type(search_method)}`."
        ...
        for constraint in constraints:
            if isinstance(
                constraint,
                textattacknew.constraints.PreTransformationConstraint,
            ):
    ...
    def cuda_(self):
        ...
        def to_cuda(obj):
            visited.add(id(obj))
            if isinstance(obj, torch.nn.Module):
                obj.to(textattacknew.shared.utils.device)
            elif isinstance(
                obj,
                (
                    Attack,
    ...
    def attack(self, example, ground_truth_output):
        ...
        ), "`example` must either be `str`, `collections.OrderedDict`, `textattacknew.shared.AttackedText`."
