...
def get_textattack_model_num_labels(model_name, model_path):
    ...
    model, if present."""
    model_cache_path = textattacknew.shared.utils.download_from_s3(model_path)
    train_args_path = os.path.join(model_cache_path, "train_args.json")
    if not os.path.exists(train_args_path):
        textattacknew.shared.logger.warn(
...
def load_textattack_model_from_path(model_name, model_path):
    ...
    if model_name.startswith("lstm"):
        ...
        model = textattacknew.models.helpers.LSTMForClassification(
        ...
        model = textattacknew.models.helpers.WordCNNForClassification(
            model_path=model_path, num_labels=num_labels
        )
    elif model_name.startswith("t5"):
        model = textattacknew.models.helpers.T5ForTextToText(model_path)
    else:
        raise ValueError(f"Unknown textattacknew model {model_path}")
    return model
