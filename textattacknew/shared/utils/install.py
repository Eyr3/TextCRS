...
def download_from_s3(folder_name, skip_if_cached=True):
    """Folder name will be saved as `<cache_dir>/textattacknew/<folder_name>`. If
...
def download_from_url(url, save_path, skip_if_cached=True):
    ...
    `<cache_dir>/textattacknew/<save_path>`. If it doesn't exist on disk, the zip
...
TEXTATTACK_CACHE_DIR = os.environ.get(
    "TA_CACHE_DIR", os.path.expanduser("~/.cache/textattacknew")
)
