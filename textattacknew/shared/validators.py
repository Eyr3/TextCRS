...
from textattacknew.goal_functions import (
    InputReduction,
    MinimizeBleu,
...
MODELS_BY_GOAL_FUNCTIONS = {
    (TargetedClassification, UntargetedClassification, InputReduction): [
        r"^textattacknew.models.helpers.lstm_for_classification.*",
        r"^textattacknew.models.helpers.word_cnn_for_classification.*",
        r"^transformers.modeling_\w*\.\w*ForSequenceClassification$",
    ],
    (NonOverlappingOutput, MinimizeBleu,): [
        r"^textattacknew.models.helpers.t5_for_text_to_text.*",
    ],
}
...
def validate_model_gradient_word_swap_compatibility(model):
    ...
    """
    if isinstance(model, textattacknew.models.helpers.LSTMForClassification):
        return True
    else:
...
def transformation_consists_of(transformation, transformation_classes):
    ...
    from textattacknew.transformations import CompositeTransformation
...
def transformation_consists_of_word_swaps(transformation):
    ...
    swaps."""
    from textattacknew.transformations import WordSwap, WordSwapGradientBased
...
def transformation_consists_of_word_swaps_and_deletions(transformation):
    """Determines if ``transformation`` is a word swap or consists of only word
    swaps and deletions."""
    from textattacknew.transformations import WordDeletion, WordSwap, WordSwapGradientBased
