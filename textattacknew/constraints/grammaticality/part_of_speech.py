...
from textattacknew.constraints import Constraint
from textattacknew.shared.utils import LazyLoader, device
from textattacknew.shared.validators import transformation_consists_of_word_swaps
        ...
        if context_key in self._pos_tag_cache:
            ...
            if self.tagger_type == "flair":
                context_key_sentence = Sentence(
                    context_key,
                    use_tokenizer=textattacknew.shared.utils.TextAttackFlairTokenizer(),
                )
                self._flair_pos_tagger.predict(context_key_sentence)
                word_list, pos_list = textattacknew.shared.utils.zip_flair_result(
                    context_key_sentence
                )

            if self.tagger_type == "stanza":
                word_list, pos_list = textattacknew.shared.utils.zip_stanza_result(
