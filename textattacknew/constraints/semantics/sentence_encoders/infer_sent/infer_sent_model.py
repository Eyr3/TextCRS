...
class InferSentModel(nn.Module):
    ...
    def forward(self, sent_tuple):
        ...
        idx_sort = (
            torch.from_numpy(idx_sort).to(textattacknew.shared.utils.device)
            if self.is_cuda()
        ...
        # Un-sort by length
        idx_unsort = (
            torch.from_numpy(idx_unsort).to(textattacknew.shared.utils.device)
            if self.is_cuda()
        ...
        if self.pool_type == "mean":
            sent_len = (
                ...
                .unsqueeze(1)
                .to(textattacknew.shared.utils.device)
            )
    ...
    def encode(self, sentences, bsize=64, tokenize=True, verbose=False):
        ...
        for stidx in range(0, len(sentences), bsize):
            ...
            if self.is_cuda():
                batch = batch.to(textattacknew.shared.utils.device)
            with torch.no_grad():
                batch = (
