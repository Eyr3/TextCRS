...
from textattacknew.shared.utils import logger
...
from .models.helpers import LSTMForClassification, WordCNNForClassification, NEWLSTMForClassification, NEWWordCNNForClassification
from .models.wrappers import ModelWrapper
from .training_args import CommandLineTrainingArgs, TrainingArgs

from .models import transformers1
...
class Trainer:
    ...
    def __init__(
        ...
        ), f"`model_wrapper` must be of type `textattacknew.models.wrappers.ModelWrapper`, but got type `{type(model_wrapper)}`."
        ...
        if attack:
            assert isinstance(
                attack, Attack
            ), f"`attack` argument must be of type `textattacknew.Attack`, but got type of `{type(attack)}`."
        ...
        if train_dataset:
            assert isinstance(
                train_dataset, textattacknew.datasets.Dataset
            ), f"`train_dataset` must be of type `textattacknew.datasets.Dataset`, but got type `{type(train_dataset)}`."

        if eval_dataset:
            assert isinstance(
                eval_dataset, textattacknew.datasets.Dataset
            ), f"`eval_dataset` must be of type `textattacknew.datasets.Dataset`, but got type `{type(eval_dataset)}`."

        if training_args:
            assert isinstance(
                training_args, TrainingArgs
            ), f"`training_args` must be of type `textattacknew.TrainingArgs`, but got type `{type(training_args)}`."
        else:
            training_args = TrainingArgs()
        ...
        if self.training_args.dynamic_mu:  # Add gaussian noise with a dynamic mu each dimension
            if 'bert' in self.training_args.model_type:
                mu_sigma = np.load('/data/xinyu/results/fgws/data/pretrained/transformers/mu_sigma.npy')
            else:
                mu_sigma = np.load('/data/xinyu/results/fgws/data/pretrained/gloVe/glove_data/mu_sigma_42B.npy')
            self.mu = torch.as_tensor(mu_sigma[:, 0]).to(textattacknew.shared.utils.device)
        # noise with adding [PAD]
        if 'bert' in self.training_args.model_type:
            self.training_args.pad_token = self.model_wrapper.tokenizer(self.training_args.pad)['input_ids'][1]
        else:
            self.training_args.pad_token = self.model_wrapper.tokenizer(self.training_args.pad)[0]
    ...
    def _generate_adversarial_examples(self, epoch):
        ...
        adversarial_dataset = textattacknew.datasets.Dataset(
            adversarial_examples,
    ...
    def _save_model_checkpoint(
        ...
        if isinstance(model, (WordCNNForClassification, LSTMForClassification, NEWLSTMForClassification, NEWWordCNNForClassification)):
            model.save_pretrained(output_dir)
        elif isinstance(model, (transformers.PreTrainedModel, transformers1.PreTrainedModel)):
            model.save_pretrained(output_dir)
    ...
    def get_optimizer_and_scheduler(self, model, num_training_steps):
        ...
        if isinstance(model, (transformers.PreTrainedModel, transformers1.PreTrainedModel)):
            # Reference https://huggingface.co/transformers/training.html
            param_optimizer = list(model.named_parameters())
            if isinstance(model, transformers1.PreTrainedModel):
                no_decay = ["embedding", "LayerNorm.bias", "LayerNorm.weight", "query.bias", "key.bias", "value.bias", "dense.bias"]
            else:
                no_decay = ["embedding", "bias", "LayerNorm.bias", "LayerNorm.weight"]  # "embedding",
            ...
            if self.training_args.learning_rate > 0.0001:
                self.training_args.learning_rate *= 0.9
                logger.info("The new learning rate is {}".format(self.training_args.learning_rate))
    ...
    def training_step(self, model, tokenizer, batch):
        ...
        if self.training_args.if_addnoise:
            input_texts_split = [input_texts[i].split(' ') for i in range(len(input_texts))]
        if self.training_args.if_addnoise == 1:
            helper.addnoise_1(input_texts_split, self.data_module.dist_mat_neighbor, self.data_module.word_to_idx,
            ...
            helper.addnoise_3(input_texts_split, self.training_args.pad)
            ...
            helper.addnoise_5(input_texts_split, self.nn_matrix, self.word2index, self.index2word, self.staircase_mech)
        elif self.training_args.if_addnoise == 6:
            helper.addnoise_6(input_texts_split, self.nn_matrix, self.word2index, self.index2word,
                              self.training_args.syn_size)
        elif self.training_args.if_addnoise == 9:  # SAFER
            helper.addnoise_9(input_texts_split, self.data_module.dist_mat_neighbor, self.data_module.word_to_idx,
                              self.data_module.idx_to_word, self.data_module.dist_mat_idx_to_idx,
        ...
        _targets = targets
        targets = targets.to(textattacknew.shared.utils.device)

        if isinstance(model, (transformers.PreTrainedModel, transformers1.PreTrainedModel)) or (
                isinstance(model, torch.nn.DataParallel) and isinstance(model.module, (transformers.PreTrainedModel, transformers1.PreTrainedModel))
        ):
            input_ids = tokenizer(
                input_texts,
                padding="max_length",
            ...
            if self.training_args.if_addnoise in [3, 7] and isinstance(model, transformers1.PreTrainedModel):
                if self.training_args.use_kl:  # v as input to the bert (without noise and CompressChannel)
                    rep_logits = model(**input_ids, noise_sd=torch.as_tensor(0).to(textattacknew.shared.utils.device),
                                       mu=self.mu, if_channel=0)[0]
                logits = model(**input_ids, noise_sd=torch.as_tensor(self.training_args.noise_sd).
                               to(textattacknew.shared.utils.device), mu=self.mu)[0]
            # elif self.training_args.if_addnoise == 3 and isinstance(model, transformers.PreTrainedModel):
            #     embeds_init = getattr(model, 'bert').embeddings.word_embeddings(input_ids.data['input_ids'])
            #     noise = torch.randn_like(embeds_init).to(textattacknew.shared.utils.device) * \
            #             torch.as_tensor(self.training_args.noise_sd).to(textattacknew.shared.utils.device)  # \
            #             # + (torch.mul(torch.ones(embeds_init.size()).cuda(), self.mu)).to(torch.float32)
            #     embeddings = embeds_init + noise
            #     logits = model(input_ids=None, token_type_ids=input_ids.data['token_type_ids'],
            #                     attention_mask=input_ids.data['attention_mask'], inputs_embeds=embeddings)[0]
            else:
                logits = model(**input_ids)[0]
        else:
            input_ids = tokenizer(input_texts)
            # if self.training_args.if_addnoise == 4:
            #     helper.addnoise_4(input_ids, self.training_args.beta, self.training_args.pad_token)  # 1917495
            if self.training_args.if_addnoise in [4, 7, 8]:
            ...
            if self.training_args.if_addnoise in [3, 7] and isinstance(model, (NEWLSTMForClassification, NEWWordCNNForClassification)):
                if self.training_args.use_kl:
                    rep_logits = model(input_ids, noise_sd=torch.as_tensor(0).to(textattacknew.shared.utils.device),
                                       mu=self.mu, if_channel=0)
                logits = model(input_ids, noise_sd=torch.as_tensor(self.training_args.noise_sd).
                               to(textattacknew.shared.utils.device), mu=self.mu)
            else:
        ...
        if self.task_type == "regression":
            ...
            # https://www.programcreek.com/python/example/104436/torch.nn.functional.kl_div
            if self.training_args.use_kl:
                vsd_loss = kl_div(input=log_softmax(logits, dim=1), target=softmax(rep_logits, dim=1), reduction='batchmean')  #
            # .detach()
        sample_weights = torch.ones(
            is_adv_sample.size(), device=textattacknew.shared.utils.device
        )
        sample_weights[is_adv_sample] *= self.training_args.alpha
        loss = loss * sample_weights
        loss = torch.mean(loss)
        if self.training_args.use_kl:
            loss += vsd_loss * self.training_args.VSD_loss
        preds = preds.cpu()

        return loss, preds, _targets, vsd_loss
    ...
    def evaluate_step(self, model, tokenizer, batch):
        ...
        if isinstance(model, (transformers.PreTrainedModel, transformers1.PreTrainedModel)):
            ...
            input_ids.to(textattacknew.shared.utils.device)
            if isinstance(model, transformers1.PreTrainedModel):
                logits = model(**input_ids, noise_sd=torch.as_tensor(0).to(textattacknew.shared.utils.device), mu=self.mu)[0]
            else:
                logits = model(**input_ids)[0]
        else:
            input_ids = tokenizer(input_texts)
            if not isinstance(input_ids, torch.Tensor):
                input_ids = torch.tensor(input_ids)
            input_ids = input_ids.to(textattacknew.shared.utils.device)
            if isinstance(model, (NEWLSTMForClassification, NEWWordCNNForClassification,)):
                logits = model(input_ids, noise_sd=torch.as_tensor(0).to(textattacknew.shared.utils.device), mu=self.mu)
            else:
                logits = model(input_ids)
    ...
    def train(self):
        ...
        textattacknew.shared.utils.set_seed(self.training_args.random_seed)
        ...
        model.to(textattacknew.shared.utils.device)

        # Variables across epochs
        self._total_loss = 0.0
        self._current_loss = 0.0
        self._total_vsd_loss = 0.0
        self._current_vsd_loss = 0.0
        ...
        for epoch in range(self.training_args.start_epoch, self.training_args.num_epochs + 1):
            logger.info("==========================================================")
            ...
            if self.attack and epoch > num_clean_epochs:
                if (
                    ...
                    # adv_dataset is instance of `textattacknew.datasets.Dataset`
                    model.eval()
            ...
            for step, batch in enumerate(prog_bar):
                loss, preds, targets, vsd_loss = self.training_step(model, tokenizer, batch)

                if isinstance(model, torch.nn.DataParallel):
                    loss = loss.mean()
                    vsd_loss = vsd_loss.mean()

                loss = loss / self.training_args.gradient_accumulation_steps
                loss.backward()
                loss = loss.item()
                if self.training_args.use_kl:
                    vsd_loss = vsd_loss.item()
                self._total_loss += loss
                self._current_loss += loss
                self._total_vsd_loss += vsd_loss
                self._current_vsd_loss += vsd_loss
                ...
                if self._global_step > 0:
                    prog_bar.set_description(
                        f"Loss {self._total_loss / self._global_step:.5f}, VSD Loss {self._total_vsd_loss / self._global_step:.5f}"
                    )
            ...
            # if best_eval_score > 0.85:
            #     logger.info("Epoch{}: Delete VSD Loss, set 'use_kl' to False, set 'VSD_loss' to 0.".format(epoch))
            #     self.training_args.use_kl = False
            #     self.training_args.VSD_loss = 0
    ...
    def evaluate(self):
        ...
        if self.training_args.test_log is not None:
            f = open(self.training_args.test_log, 'a+')
            if self._metric_name == "accuracy":
                print("Eval {}: {}%".format(self._metric_name, eval_score), file=f, flush=True)
            else:
                print("Eval {}: {.4}%".format(self._metric_name, eval_score), file=f, flush=True)
            f.close()

        return eval_score

    def predict(self):
        """Predict(evaluate) the model on given evaluation dataset."""

        if not self.eval_dataset:
            raise ValueError("No `eval_dataset` available for training.")

        logging.info("Predicting model on evaluation dataset.")
        model = self.model_wrapper.model
        tokenizer = self.model_wrapper.tokenizer

        alpha = 0.001
        N = 20000  # self.training_args.certify_batch * 100
        ...
        if isinstance(model, torch.nn.DataParallel):
            num_gpus = torch.cuda.device_count()
            eval_batch_size = 1 * num_gpus  # self.training_args.per_device_eval_batch_size
        else:
            eval_batch_size = 1  # self.training_args.per_device_eval_batch_size

        eval_dataloader = self.get_eval_dataloader(self.eval_dataset, eval_batch_size)

        smoothed_classifier = Smooth(model, tokenizer, self.training_args, self.data_module, self.staircase_mech,
                                     self.nn_matrix, self.word2index, self.index2word, self.mu)
        f = open(self.training_args.certify_log, 'a+')
        print('N={}'.format(N), file=f, flush=True)
        with torch.no_grad():
            for step, batch in enumerate(eval_dataloader):
                if step % self.training_args.skip != 0:
                    continue

                input_texts, targets = batch
                # targets = targets.item()
                preds = smoothed_classifier.spredict(input_texts, N, alpha)
                all_preds.append(preds)
                all_targets.append(targets.item())
                if step % (self.training_args.skip * 100) == 0:
                    print('Have evaluated {} steps.'.format(step/self.training_args.skip), file=f, flush=True)

        preds = np.array(all_preds).reshape(-1, 1)  # torch.cat
        targets = np.array(all_targets).reshape(-1, 1)
        correct_predictions = (preds == targets).sum()
        accuracy = correct_predictions / len(targets)
        eval_score = accuracy

        print("Evaluate on {} adversarial examples, N={}".format(math.ceil(step / self.training_args.skip), N), file=f,
              flush=True)
        if self._metric_name == "accuracy":
            print(f"Eval {self._metric_name}: {eval_score * 100:.2f}%", file=f, flush=True)
        else:
            print(f"Eval {self._metric_name}: {eval_score:.4f}%", file=f, flush=True)
        f.close()
        return eval_score

    def certify(self):
        """Certify the model on given evaluation dataset."""

        if not self.eval_dataset:
            raise ValueError("No `eval_dataset` available for training.")

        logging.info("Certifying model on evaluation dataset.")
        model = self.model_wrapper.model
        tokenizer = self.model_wrapper.tokenizer

        N0 = 100
        alpha = 0.001
        N = self.training_args.N  # 100000, 20000
        ...
        smoothed_classifier = Smooth(model, tokenizer, self.training_args, self.data_module, self.staircase_mech,
                                     self.nn_matrix, self.word2index, self.index2word, self.mu)
        if self.training_args.continue_idx == -1:
            f = open(self.training_args.certify_log, 'w')
            if self.training_args.if_addnoise in [1, 2, 3, 5, 8]:  # no radius formulas
                print("idx\tlabel\tpredict\tradius\tpBBar\tcorrect\ttime", file=f, flush=True)  #
            else:  # 4 and other
                print("idx\tlabel\tpredict\tpABar\tpBBar\tcorrect\ttime", file=f, flush=True)  #
        else:
            f = open(self.training_args.certify_log, 'a+')
        #     f.write('\n')
        with torch.no_grad():
            total_cnt = 0
            for step, batch in enumerate(eval_dataloader):
                if (step <= self.training_args.continue_idx) or (step % self.training_args.skip != 0):
                    continue

                input_texts, targets = batch
                # targets = targets.item()
                before_time = time()
                preds, radius, pBBar = smoothed_classifier.scertify(input_texts, N0, N, alpha)
                after_time = time()

                correct = int(preds == targets)
                time_elapsed = str(datetime.timedelta(seconds=(after_time - before_time)))
                print("{}\t{}\t{}\t{:.5}\t{:.5}\t{}\t{}".format(  #
                    step, targets, preds, radius, pBBar, correct, time_elapsed), file=f, flush=True)  #
                total_cnt += 1
                if total_cnt == 500:
                    break
        f.close()
    ...
    def _write_readme(self, best_eval_score, best_eval_score_epoch, train_batch_size):
        ...
        elif isinstance(self.model_wrapper.model, (transformers.PreTrainedModel, transformers1.PreTrainedModel)):
            if (
        ...
        if (
            ...
            model_max_length = self.training_args.model_max_length
                ...
                (
                        ...
                        transformers1.PreTrainedModel,
                        LSTMForClassification,
                        NEWLSTMForClassification,
                        WordCNNForClassification,
                        NEWWordCNNForClassification,
                ),
        ):
        ...
        if isinstance(
                self.train_dataset, textattacknew.datasets.HuggingFaceDataset
            ...
            if hasattr(self.train_dataset, "_subset"):
                ...
                self.eval_dataset, textattacknew.datasets.HuggingFaceDataset
