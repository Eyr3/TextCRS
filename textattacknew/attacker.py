...
from textattacknew.attack_results import (
    FailedAttackResult,
...
class Attacker:
    ...
    Example::
        ...
        >>> attack = textattacknew.attack_recipes.TextFoolerJin2019.build(model_wrapper)
        >>> dataset = textattacknew.datasets.HuggingFaceDataset("imdb", split="test")
    ...
    def __init__(self, attack, dataset, attack_args=None):
        ...
        assert isinstance(
            dataset, textattacknew.datasets.Dataset
        ), f"`dataset` must be of type `textattacknew.datasets.Dataset`, but got type `{type(dataset)}`."

        if attack_args:
            assert isinstance(
                attack_args, AttackArgs
            ), f"`attack_args` must be of type `textattacknew.AttackArgs`, but got type `{type(attack_args)}`."
        else:
            attack_args = AttackArgs()
    ...
    def _attack(self):
        ...
        while worklist:
            ...
            try:
                ...
                continue
            example = textattacknew.shared.AttackedText(example)
            ...
            if (
                ...
                new_checkpoint = textattacknew.shared.AttackCheckpoint(
                    self.attack_args,
    ...
    def attack_dataset(self):
        ...
        Returns:
            :obj:`list[AttackResult]` - List of :class:`~textattacknew.attack_results.AttackResult` obtained after attacking the given dataset..
        """
        ...
        textattacknew.shared.utils.set_seed(self.attack_args.random_seed)
    ...
    def update_attack_args(self, **kwargs):
        ...
        for k in kwargs:
            if hasattr(self.attack_args, k):
                ...
                raise ValueError(f"`textattacknew.AttackArgs` does not have field {k}.")
    ...
    def from_checkpoint(cls, attack, dataset, checkpoint):
        ...
        Args:
            ...
            dataset (:class:`~textattack.datasets.Dataset`):
                Dataset to attack.
            checkpoint (:obj:`Union[str, :class:`~textattacknew.shared.AttackChecpoint`]`):
                Path of saved checkpoint or the actual saved checkpoint.
        """
        assert isinstance(
            checkpoint, (str, textattacknew.shared.AttackCheckpoint)
        ), f"`checkpoint` must be of type `str` or `textattacknew.shared.AttackCheckpoint`, but got type `{type(checkpoint)}`."

        if isinstance(checkpoint, str):
            checkpoint = textattacknew.shared.AttackCheckpoint.load(checkpoint)
    ...
    def attack_interactive(attack):
        ...
        while True:
            ...
            example = textattacknew.shared.attacked_text.AttackedText(text)
...
def attack_from_queue(
    ...
    textattacknew.shared.utils.set_seed(attack_args.random_seed)
