...
CUDA_VISIBLE_DEVICES=$GPU python nohup -u textatk_train.py -mode $MODE -model_type $MODEL_TYPE -dataset $DATASET -if_addnoise $NOISE_TYPE -syn_size $PARAMETER -sigma $PARAMETER -atk $ATTACK -ae_data $ATTACK -model_path /data/xinyu/results/fgws/models/${MODEL_TYPE}/${DATASET}/noise${NOISE_TYPE}_k=${PARAMETER}/best_model/ >/dev/null 2>&1 &
